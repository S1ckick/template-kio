...
\begin{document}
    ...
    % --- Additional IEEE 2025 reference types ---
    % Six book-layout variants (IEEE 2025 Section B):
    Plain book (author, title, address, publisher, year)~--- \cite{IEEEKlausHorn}.
    Book chapter with edition, chapter, section, and page range~--- \cite{IEEEOguraChapter}.
    Multi-author book with several editors (Eds. + ``et al.'')~--- \cite{IEEEMultiAuthorMultiEditor}.
    Chapter with author-side ``et al.'', editor, edition, and ``(in Language)''~--- \cite{IEEEPontryaginMaxPrinciple}.
    Chapter written by the volume editor; foreign-language publisher, DOI, URL~--- \cite{IEEESanchezEditorAuthored}.
    Chapter with series in parentheses, two editors, a translator, and edition~--- \cite{IEEESeriesTranslatorChapter}.
    Book chapter with editor and series~--- \cite{IEEEFeistauer}.
    Translated book~--- \cite{IEEEDessalles}.
    Periodical with article ID (Art.~no.)~--- \cite{IEEEFardel}.
    Periodical in a non-English language (with the ``(in Language)'' marker)~--- \cite{IEEEWignerGerman}.
    Blog post~--- \cite{IEEEOwens}.
    Conference paper presented at a conference~--- \cite{IEEECaratelli}.
    Conference proceedings paper~--- \cite{IEEEAmador}.
    Online conference proceedings paper~--- \cite{IEEESchubertDAC}.
    Conference paper online~--- \cite{IEEETaylorConfOnline}.
    Online dataset~--- \cite{IEEEHHSDataset}.
    Print handbook / manual~--- \cite{IEEETransmissionSystems}.
    Online manual~--- \cite{IEEEBreimannRF}.
    Lecture notes~--- \cite{IEEEBarneyLecture}.
    Online news article~--- \cite{IEEEMarkoffNews}.
    Online video / YouTube~--- \cite{IEEEmtaOnlineVideo}.
    Patent (print)~--- \cite{IEEEWilkinsonPatent}.
    Patent (online)~--- \cite{IEEEWilliamsPatentOnline}.
    Software~--- \cite{IEEEArningSoftware}.
    IEEE standard~--- \cite{IEEEStd308}.
    IEEE standard with every field populated (location, date, DOI, URL)~--- \cite{IEEEStdIEEE754Full}.
    Master's thesis~--- \cite{IEEEKawasakiThesis}.
    Ph.D. dissertation~--- \cite{IEEEWilliamsPhD}.
    Government document~--- \cite{IEEEUSHouseGov}.
    arXiv preprint~--- \cite{IEEEUrazhdinArxiv}.
    Website~--- \cite{IEEEKumarWebsite}.
    Social media~--- \cite{IEEECostanzoTwitter}.
    ...
    \begin{otherlanguage*}{english}
        ...
        \bibliography{references-en-prefixed,references-clones}
    \end{otherlanguage*}
