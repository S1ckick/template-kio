...
\begin{document}
    ...
    Plain book (author, title, address, publisher, year)~--- \cite{KlausHorn}.
    Book chapter with edition, chapter, section, and page range~--- \cite{OguraChapter}.
    Multi-author book with several editors (Eds. + ``et al.'')~--- \cite{MultiAuthorMultiEditor}.
    Chapter with author-side ``et al.'', editor, edition, and ``(in Language)''~--- \cite{PontryaginMaxPrinciple}.
    Chapter written by the volume editor; foreign-language publisher, DOI, URL~--- \cite{SanchezEditorAuthored}.
    Chapter with series in parentheses, two editors, a translator, and edition~--- \cite{SeriesTranslatorChapter}.
    Book chapter with editor and series~--- \cite{Feistauer}.
    Translated book~--- \cite{Dessalles}.
    Periodical with article ID (Art.~no.)~--- \cite{Fardel}.
    Periodical in a non-English language (with the ``(in Language)'' marker)~--- \cite{WignerGerman}.
    Blog post~--- \cite{Owens}.
    Conference paper presented at a conference~--- \cite{Caratelli}.
    Conference proceedings paper~--- \cite{Amador}.
    Online conference proceedings paper~--- \cite{SchubertDAC}.
    Conference paper online~--- \cite{TaylorConfOnline}.
    Online dataset~--- \cite{HHSDataset}.
    Print handbook / manual~--- \cite{TransmissionSystems}.
    Online manual~--- \cite{BreimannRF}.
    Lecture notes~--- \cite{BarneyLecture}.
    Online news article~--- \cite{MarkoffNews}.
    Online video / YouTube~--- \cite{IEEEmtaOnlineVideo}.
    Patent (print)~--- \cite{WilkinsonPatent}.
    Patent (online)~--- \cite{WilliamsPatentOnline}.
    Software~--- \cite{ArningSoftware}.
    IEEE standard~--- \cite{Std308}.
    IEEE standard with every field populated (location, date, DOI, URL)~--- \cite{StdIEEE754Full}.
    Master's thesis~--- \cite{KawasakiThesis}.
    Ph.D. dissertation~--- \cite{WilliamsPhD}.
    Government document~--- \cite{USHouseGov}.
    arXiv preprint~--- \cite{UrazhdinArxiv}.
    Website~--- \cite{KumarWebsite}.
    Social media~--- \cite{CostanzoTwitter}.
