...
\begin{document}
    ...
    % --- Примеры базовых типов литературы ---
    ...
    % --- Дополнительные типы литературы по IEEE 2025 (раздел II гайда) ---
    % Шесть вариантов оформления книг (раздел B руководства IEEE 2025):
    Простейшая книга (автор, заголовок, город, издатель, год)~--- \cite{KlausHorn}.
    Глава книги с указанием издания, главы, раздела и страниц~--- \cite{OguraChapter}.
    Книга нескольких авторов с многими редакторами (Eds. + <<et al.>>)~--- \cite{MultiAuthorMultiEditor}.
    Глава с <<автор et al.>>, редактором, изданием и пометкой <<(in Language)>>~--- \cite{PontryaginMaxPrinciple}.
    Глава, написанная редактором тома; издатель <<(in Language)>>, с DOI и URL~--- \cite{SanchezEditorAuthored}.
    Глава с серией в скобках, двумя редакторами, переводчиком и изданием~--- \cite{SeriesTranslatorChapter}.
    Глава книги с редактором и серией~--- \cite{Feistauer}.
    Переведённая книга~--- \cite{Dessalles}.

    % Periodicals
    Статья в журнале на другом языке (с пометкой <<(in Language)>>)~--- \cite{WignerGerman}.

    % Periodical with article ID
    Статья в журнале с идентификатором (Art.~no.)~--- \cite{Fardel}.
    

    Блог~--- \cite{Owens}.
    Доклад на конференции (без публикации в сборнике)~--- \cite{Caratelli}.
    Статья в трудах конференции~--- \cite{Amador}.
    Статья в трудах конференции онлайн~--- \cite{SchubertDAC}.
    Электронный доклад на конференции~--- \cite{TaylorConfOnline}.
    Набор данных в сети~--- \cite{HHSDataset}.
    Печатное руководство (Manual)~--- \cite{TransmissionSystems}.
    Электронное руководство~--- \cite{BreimannRF}.
    Лекционные материалы~--- \cite{BarneyLecture}.
    Новостная статья~--- \cite{MarkoffNews}.
    Видеоролик на YouTube~--- \cite{mtaOnlineVideo}.
    Русский видеоролик (VK Video, ГОСТ-стиль)~--- \cite{vkVideoExample}.
    Патент~--- \cite{WilkinsonPatent}.
    Электронный патент~--- \cite{WilliamsPatentOnline}.
    Программное обеспечение~--- \cite{ArningSoftware}.
    Российское программное обеспечение (ГОСТ-стиль)~--- \cite{schoolPassportSoftware}.
    Стандарт IEEE~--- \cite{Std308}.
    Стандарт IEEE со всеми полями (адрес, дата, DOI, URL)~--- \cite{StdIEEE754Full}.
    Магистерская диссертация~--- \cite{KawasakiThesis}.
    Кандидатская/PhD-диссертация~--- \cite{WilliamsPhD}.
    Документ правительства~--- \cite{USHouseGov}.
    Препринт arXiv~--- \cite{UrazhdinArxiv}.
    Веб-сайт~--- \cite{KumarWebsite}.
    Социальная сеть~--- \cite{CostanzoTwitter}.
